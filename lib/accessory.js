...
module.exports = function (homebridge) {
  ...
  class MilightAccessory {
    constructor (logger, config) {
      ...
      this.bulb = this.hub.bulb(config.type, config.id, config.group, null);

      this.name = String(config.name);
      this.id = UUIDGen.generate('Milight Local' + (this.name || '') + this.bulb.identifier);

      this.services = {};
      this.reachable = true;

      // for async mode, we need to enable polling based synchronisation
    ...
    get displayName () {
      return this.name || `${MilightAccessory.ACCESSORY_NAME} ${this.bulb.identifier}`;
    }
    ...
    getServices () {
      ...
      if (!this.services.lightbulb) {
        ...
        MilightAccessory.configureLightbulbService(this.bulb, this.services.lightbulb);
      }
    ...
    static configureLightbulbService(bulb, lightbulb) {
      ...
      bulb.white && lightbulb.getCharacteristic(Characteristic.ColorTemperature)
        .on('get', (callback) => {
      ...
      bulb.rgb && lightbulb.getCharacteristic(Characteristic.Hue)
        .on('get', bulb.getHue.bind(bulb))
        .on('set', bulb.setHue.bind(bulb));

      bulb.rgb && lightbulb.getCharacteristic(Characteristic.Saturation)
    ...
    static createLightbulbService (bulb, name) {
      ...
      // lightbulb.addCharacteristic(Characteristic.On); // added default
      lightbulb.addCharacteristic(Characteristic.Brightness);

      // @todo have a better api to work on modes. this is error prone
      if (bulb.white) {
    ...
    static updateLightbulbCharacteristics (lightbulb, bulb) {
      for (let [prop, characteristic] of Object.entries(MilightAccessory.characteristics)) {
        lightbulb.getCharacteristic(characteristic).updateValue(bulb[prop]);
      }
    }
  }
