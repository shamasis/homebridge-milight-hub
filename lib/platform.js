...
module.exports = function (homebridge) {
  ...
  class MilightPlatformAccessory extends Accessory {
    constructor (bulb, logger = console) {
      ...
      MilightAccessory.configureLightbulbService(this.bulb, this.getService(Service.Lightbulb));
    ...
    static upgradePlatformAccessory (accessory, hub, logger) {
      accessory.bulb = hub.bulb(...accessory.context.bulb);
      accessory.name = accessory.context.name;
      accessory.reachable = true;
      accessory.log = logger;
      MilightAccessory.configureLightbulbService(accessory.bulb, accessory.getService(Service.Lightbulb));
      accessory.log.debug('%s cached accessory was instantiated.', accessory.name);
    }
  }
  ...
  class MilightPlatform {
    ...
    createAccessory (bulb) {
      ...
      // configure property update polling
    ...
    configureAccessory (accessory) { // @todo build a way to augment base accessory to the desired class
      ...
      if (accessory.context && accessory.context.identifier) {
        ...
        MilightPlatformAccessory.upgradePlatformAccessory(accessory, this.hub, this.log);
        MilightAccessory.enableAutoSynchronise(accessory.getService(Service.Lightbulb), accessory.bulb);

        if (accessory.context.name) {
          accessory.displayName = accessory.context.name;
        }
      }
