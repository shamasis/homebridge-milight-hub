const
  ...
  /**
   * List of power states that can be communicated with the hub. Milight hub can accept both true/false and on/off,
   * while it responds as on/off alone. This enum also helps normalise the same.
   * @enum
   * @private
   */
  POWER_STATES = {
  ...
  /**
   * This captures the possible mode of the bulb. Certain bulbs have two separate modes they run as. Colour and White.
   * There are places where logic has to be handled differently for different modes.
   * @enum
   * @private
   */
  BULB_MODES = {
  ...
  /**
   * While sending commands to hub via REST API, certain commands are not treated as root level keys, but instead has to
   * be sent as array of commands MQTT style. This enum helps to lookup such commands and normalise
   * @enum
   * @private
   */
  QUEUABLE_COMMANDS = {
...
class MilightBulb {
   ...
   * @param {String} group
   * @param {?String} [alias]
   * @param {MilightHub} hub
   */
  constructor (type, id, group, alias, hub) {
    this.type = type;
    this.id = id;
    this.group = group;
    this.alias = alias;
    ...
    this.log = hub.log;
    this.context = {};
    this.synchronisedAt = 0;
  }

  reconstructor () {
    return [this.type, this.id, this.group, this.alias];
  }
  ...
  synchronise (state) {
    this.synchronisedAt = Date.now();
    Object.entries(state).forEach(entry => this[entry[0]] = entry[1]);
    return state;
  }

  state (done) {
    this.log.debug('%s to fetch current state', this.identifier);

    this.hub.state(this.id, this.type, this.group, (err, state) => {
      if (err) { return done(err); }
      if (!state) { return done(new Error('State not received')); }

      this.log.debug('%s responded with current state', this.identifier);
      done(null, this.synchronise(state));
    });
  }

  command (key, value, done) {
    this.log.debug('%s to update %s=%s', this.identifier, key, value);

    this.hub.command(key, value, this.id, this.type, this.group, (err, state) => {
      if (err) {
        this.log.error('%s unable to update %s=%s', this.identifier, key, value);
        this.log.error(err);
        return done(err);
      }

      if (!state) {
        this.log.error('%s returned unexpected http response while expecting state data.', this.identifier);
        return done(new Error('State not received'));
      }

      state = this.synchronise(state);

      this.log.debug('%s responded to %s=%s with state',this.identifier, key, value, state);
  ...
  setColourTemperature (value, done) {
    ...
    if (this.mode !== BULB_MODES.white) {
      this.log.info('%s switching bulb mode to "white" from "%s"', this.identifier, this.mode);

      return this.command('set_white', null, (err) => {
        if (err) { return done(err); }
        this.mode = BULB_MODES.white; // force to avoid recursion
...
class MilightHub {
   ...
   */
  constructor (host = 'http://milight-hub.local', timeout = 30000, logger = console) {
    /**
    ...
    this.log.info('"%s" hub initialised', this.host);
  }
  ...
  static parse (body) {
    ...
    if (body.hasOwnProperty('success')) {
      state.success = Boolean(body.success);
    }
  ...
  information (done) {
    this.request.get('/about', (err, res, body) => {
      if (err) { return done(err); }
      if (!body) { return done(new Error('Invalid hub response while fetching hub information.')); }

      if (res && res.statusCode !== 200) {
        return done(new Error('Unexpected response code from hub while fetching hub settings.'));
      }
  ...
  devices (done) {
    this.request.get('/settings', (err, res, body) => {
      if (err) { return done(err); }
      if (!body) { return done(new Error('Invalid hub response while fetching devices.')); }

      if (res && res.statusCode !== 200) {
        return done(new Error('Unexpected response code while fetching list of devices'));
      }
  ...
  bulb (type, id, group, alias) {
    return new MilightBulb(type, id, group, alias, this);
  }
  ...
  bulbs (done) {
    this.devices((err, bulbs) => {
      if (err) { return done(err); }
      done(null, bulbs.map((bulb) => this.bulb(bulb.type, bulb.id, bulb.group, bulb.alias)));
    })
  }
  ...
  state (id, type, group, done) {
    this.request.get(`/gateways/${id}/${type}/${group}?blockOnQueue=true`, (err, res, body) => {
      ...
      if (res && res.statusCode !== 200) {
        return done(new Error('Unexpected response code from hub while receiving state.'));
      }

      if (res && res.statusCode !== 200) {
        return done(new Error('Unexpected response code from hub while fetching state.'));
      }
  ...
  command (key, value, id, type, group, done) {
    ...
    this.request.put(`/gateways/${id}/${type}/${group}?blockOnQueue=true`, {
      ...
      if (res && res.statusCode !== 200) {
        return done(new Error('Unexpected response code from hub while receiving command result.'));
      }

      done(null, MilightHub.parse(body), value, key);
    });
  }
};
