...
class MilightBulb {
   ...
   */
  constructor (alias, type, id, group, hub, logger = console, async) {
    this.alias = alias;
    this.type = type;
    this.id = id;
    this.group = group;
  ...
  command (key, value, done) {
    ...
    this.hub.command(key, value, this.id, this.type, this.group, (err, state) => {
      ...
      if (!state) {
        this.log.error('%s returned unexpected http response while expecting state data.');
        return done(new Error('State not received'));
      }

      // synchronise context with state response
      if (!this.async) {
        state = this.synchronise(state);
        this.log.debug('%s responded to %s=%s with state',this.identifier, key, value, state);
      }
      else {
        this.log.debug('%s responded to %s=%s with success: %s',this.identifier, key, value, state.success);
      }
...
class MilightHub {
  ...
  bulbs (done) {
    this.devices((err, bulbs) => {
      if (err) { return done(err); }
      done(null, bulbs.map((bulb) => new MilightBulb(bulb.alias, bulb.type, bulb.id, bulb.group, this, this.log)));
    })
  }
   ...
   * @param {Function} done
   */
  state (id, type, group, done, wait = true) {
    this.request.get(`/gateways/${id}/${type}/${group}${(wait ? '?blockOnQueue=true' : '')}`, (err, res, body) => {
      if (err) { return done(err); }
   ...
   * @param {Function} done
   */
  command (key, value, id, type, group, done, wait = true) {
    ...
    this.request.put(`/gateways/${id}/${type}/${group}${(wait ? '?blockOnQueue=true' : '')}`, {
      json: command
    }, (err, res, body) => {
      if (err) { return done(err); }
      if (!body) { return done(new Error('Did not receive state from hub.')); }
      ...
      if (res && res.statusCode !== 200) {
        return done(new Error('Unexpected response code from hub while sending command.'));
      }
